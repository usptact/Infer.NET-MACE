...
> **Status: ported from the `online-mace` branch, and kept as an audit record**
>
> This document was written for the ThreatSense deployment, where MACE served sensor
> readings and threat levels. The code on this branch is the domain-neutral library —
> workers, items and labels — with the service mapping domain terms at its edge.
> Read sensor as worker, incident as item and threat level as label.
>
> Two claims in the original are now known to be wrong and are corrected here rather
> than in the text below:
>
> - **Inference is expectation propagation, not VMP.** The generated algorithm has always
>   been `Model_EP`. Switching the engine to VMP fails outright with "The model has zero
>   probability": the non-spammer branch copies the true label deterministically, so under
>   VMP any two annotators who disagree multiply to zero. See `MACEBase.CreateModel`.
> **Issue 3 (no forgetting / decay) is resolved on this branch.** `PriorUpdateService` takes a
> `retention` parameter that decays accumulated evidence toward the uninformative prior, bounding it
> at `learningRate / (1 - retention)`; the service defaults it to 0.99. Issues 6 and 7 remain open.
>
> - **The gRPC surface has changed.** `Infer` / `UpdatePriors` are now `InferLabel` /
>   `SubmitFeedback` / `GetWorkerReliability`, and the service owns worker parameters
>   instead of taking priors in and out on every call. `MACE.Service/Protos/mace_inference.proto`
